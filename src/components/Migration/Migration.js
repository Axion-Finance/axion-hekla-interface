...
import LiqMigrator from "abis/LiqMigrator.json";
...
const tokens = [
  {
    ...
    iouToken: getContract(CHAIN_ID, "GMT_LIQ_IOU"),
    cap: MaxUint256,
    bonus: 0,
  },
  {
    ...
    iouToken: getContract(CHAIN_ID, "XGMT_LIQ_IOU"),
    cap: MaxUint256,
    bonus: 0,
  },
  {
    ...
    iouToken: getContract(CHAIN_ID, "GMT_USDG_LIQ_IOU"),
  ...
  },
  {
    ...
    iouToken: getContract(CHAIN_ID, "XGMT_USDG_LIQ_IOU"),
...
const liqMigratorAddress = getContract(CHAIN_ID, "LiqMigrator");
...
function MigrationModal(props) {
  ...
  const { data: tokenAllowance, mutate: updateTokenAllowance } = useSWR(
    [active, CHAIN_ID, token.address, "allowance", account, liqMigratorAddress],
    {
  ...
  const onClickPrimary = () => {
    ...
    setIsMigrating(true);
    const contract = new ethers.Contract(liqMigratorAddress, LiqMigrator.abi, library.getSigner());
    contract
...
export default function Migration() {
  ...
  const { data: migratedAmounts, mutate: updateMigratedAmounts } = useSWR(
    ["Migration:migratedAmounts", CHAIN_ID, liqMigratorAddress, "getTokenAmounts"],
    {
      fetcher: contractFetcher(library, LiqMigrator, [tokenAddresses]),
    }
  );

  let liqBalance;
  let totalMigratedLiq;
  let totalMigratedUsd;

  if (iouBalances) {
    liqBalance = bigNumberify(0);
    totalMigratedLiq = bigNumberify(0);

    for (let i = 0; i < iouBalances.length / 2; i++) {
      liqBalance = liqBalance.add(iouBalances[i * 2]);
            ...
            <div className={cx("border", "App-card", { primary: index === 0 })} key={index}>
              ...
              <div className="Stake-card-bottom App-card-content">
                ...
                <div className="App-card-options">
                  ...
                  {active && (
                    <button className="default-btn App-card-option" onClick={() => showMigrationModal(index)}>
                      <Trans>Migrate</Trans>
                    </button>
                  )}
                </div>
