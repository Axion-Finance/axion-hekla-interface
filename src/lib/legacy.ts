...
export const MAX_LEVERAGE = 150 * BASIS_POINTS_DIVISOR;
...
export const LIQ_DECIMALS = 18;
export const DEFAULT_MAX_USDG_AMOUNT = expandDecimals(200 * 1000 * 1000, 18);

export const TAX_BASIS_POINTS = 60;
export const STABLE_TAX_BASIS_POINTS = 60;
...
export function getStakingData(stakingInfo) {
  ...
  const keys = ["stakedLiqTracker", "bonusLiqTracker", "feeLiqTracker", "stakedGlpTracker", "feeGlpTracker"];
  const data = {};
  const propsLength = 5;
...
export function getProcessedData(
  ...
  // data.feeLiqTrackerRewards = stakingData.feeLiqTracker.claimable;
  // data.feeLiqTrackerRewardsUsd = stakingData.feeLiqTracker.claimable.mul(nativeTokenPrice).div(expandDecimals(1, 18));

  // data.boostBasisPoints = bigNumberify(0);
  // if (data && data.bnLiqInFeeLiq && data.bonusLiqInFeeLiq && data.bonusLiqInFeeLiq.gt(0)) {
  //   data.boostBasisPoints = data.bnLiqInFeeLiq.mul(BASIS_POINTS_DIVISOR).div(data.bonusLiqInFeeLiq);
  // }
  ...
  //     ? data.stakedLiqTrackerAnnualRewardsUsd.mul(BASIS_POINTS_DIVISOR).div(data.stakedLiqTrackerSupplyUsd)
  //     : bigNumberify(0);
  // data.feeLiqTrackerAnnualRewardsUsd = stakingData.feeLiqTracker.tokensPerInterval
  //   .mul(SECONDS_PER_YEAR)
  //   .mul(nativeTokenPrice)
  //   .div(expandDecimals(1, 18));
  // data.liqAprForNativeToken =
  //   data.feeLiqSupplyUsd && data.feeLiqSupplyUsd.gt(0)
  //     ? data.feeLiqTrackerAnnualRewardsUsd.mul(BASIS_POINTS_DIVISOR).div(data.feeLiqSupplyUsd)
  //     : bigNumberify(0);
  // data.liqBoostAprForNativeToken = data.liqAprForNativeToken.mul(data.boostBasisPoints).div(BASIS_POINTS_DIVISOR);
  // data.liqAprTotal = data.liqAprForNativeToken.add(data.liqAprForEsLiq);
  // data.liqAprTotalWithBoost = data.liqAprForNativeToken.add(data.liqBoostAprForNativeToken).add(data.liqAprForEsLiq);
  // data.liqAprForNativeTokenWithBoost = data.liqAprForNativeToken.add(data.liqBoostAprForNativeToken);
  ...
  data.glpAprForNativeToken =
    data.glpSupplyUsd && data.glpSupplyUsd.gt(0)
      ...
      : bigNumberify(0);
  data.glpAprTotal = data.glpAprForNativeToken.add(data.glpAprForEsLiq);

  data.totalGlpRewardsUsd = data.stakedGlpTrackerRewardsUsd.add(data.feeGlpTrackerRewardsUsd);

  // data.totalEsLiqRewards = data.stakedLiqTrackerRewards.add(data.stakedGlpTrackerRewards);
  // data.totalEsLiqRewardsUsd = data.stakedLiqTrackerRewardsUsd.add(data.stakedGlpTrackerRewardsUsd);
  ...
  // data.totalRewardsUsd = data.totalEsLiqRewardsUsd.add(data.totalNativeTokenRewardsUsd).add(data.totalVesterRewardsUsd);
  return data;
}
...
export function getAppBaseUrl() {
  ...
  return "https://testnet.axion.finance/#";
}
...
export function getTradePageUrl() {
  ...
  return "https://testnet.axion.finance/#/trade";
}
